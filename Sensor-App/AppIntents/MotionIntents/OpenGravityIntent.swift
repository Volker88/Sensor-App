...
nonisolated struct OpenGravityIntent: AppIntent {
    ...
    static let title = LocalizedStringResource("Open Gravity", table: "AppIntents")

    static var description: IntentDescription? {
        IntentDescription(
            LocalizedStringResource("Open Gravity", table: "AppIntents"),
        ...
        if let appState {
            if appState.isIphone {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    appState.motionStack = [.gravity]
                }
            } else {
                appState.selectedTab = .gravity
            }
        }
